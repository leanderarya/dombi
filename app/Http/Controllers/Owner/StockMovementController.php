...
class StockMovementController extends Controller
{
    public function index(Request $request): Response
    {
        $movements = StockMovement::query()
            ->with(['outlet', 'variant.family', 'creator'])
        ...
        return Inertia::render('owner/stock-movements/index', [
            ...
            'families' => ProductFamily::with(['variants' => fn ($q) => $q->where('is_active', true)->orderBy('name')])->orderBy('name')->get(),
            'filters' => $request->only(['outlet_id', 'product_variant_id', 'type', 'date_from', 'date_to']),
        ]);
    }
}
