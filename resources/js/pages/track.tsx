...
import { ArrowLeft, Clock, Copy, MapPin, MessageCircle, Package, RotateCcw, Share2, Store, Truck, UserCheck, XCircle, CheckCircle2, Circle } from 'lucide-react';
...
const TIMELINE_STEPS = [
    ...
    { key: 'ready_for_pickup', label: 'Pesanan Siap Diambil', icon: Package },
...
const PICKUP_TIMELINE_STEPS = [
    ...
    { key: 'confirmed', label: 'Outlet Menerima Pesanan', icon: Package },
    { key: 'preparing', label: 'Pesanan Sedang Disiapkan', icon: Package },
    { key: 'ready_for_pickup', label: 'Siap Diambil di Outlet', icon: Package },
    { key: 'completed', label: 'Pesanan Diambil', icon: CheckCircle2 },
];
                ...
                {order.outlet && (
                    <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                        <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400">
                            {order.fulfillment_type === 'pickup' ? 'Pengambilan' : 'Pengiriman'}
                        </div>
                        <div className="mt-2 flex items-center gap-2">
                            <Store className="h-4 w-4 text-slate-400" />
                            <span className="text-sm font-semibold text-slate-900">{order.outlet.name}</span>
                        </div>
                        
                        {order.fulfillment_type === 'pickup' && (
                            <div className="mt-3 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2">
                                <div className="text-xs font-semibold text-blue-800">Ambil di Outlet</div>
                                <div className="mt-1 text-xs text-blue-700">
                                    Pesanan Anda sudah siap. Silakan ambil di outlet dengan menunjukkan kode pesanan.
                                </div>
                                {order.status === 'ready_for_pickup' && (
                                    <div className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-blue-800">
                                        <MapPin className="h-3.5 w-3.5" />
                                        <span>Tunjukkan kode pesanan ke kasir</span>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {order.delivery?.courier && (
                            <div className="mt-2 text-xs text-slate-500">Kurir: {order.delivery.courier.name}</div>
                        )}
                        {order.customer_address && order.fulfillment_type !== 'pickup' && (
