    ...
    return (
        <OutletLayout title="Ajukan Return" subtitle="Return stok outlet" backHref="/outlet/returns" hideNav>
            ...
            <div className="mt-4 pb-40">
                {/* Reason */}
                <div>
                    <label className="text-sm font-semibold text-text">Alasan Return</label>
                    <select
                ...
                {/* Evidence Photos */}
                <div className="mt-4">
                    <label className="text-sm font-semibold text-text">Foto Bukti (Opsional)</label>
                    <p className="mt-1 text-xs text-text-muted">Upload foto kerusakan/masalah barang (maks. 5 foto)</p>
                ...
                {/* Variant Selection */}
                <div className="mt-6">
                    <label className="text-sm font-semibold text-text">Pilih Produk</label>
                    <div className="mt-2 space-y-2">
                        {variants.map((v: any) => {
                            ...
                            return (
                                <div key={v.id} className={`rounded-xl border p-3 transition-colors ${isSelected ? 'border-primary bg-primary-light' : 'border-border bg-white'}`}>
                                    <button onClick={() => toggleVariant(v.id)} disabled={availableStock <= 0} className="flex min-h-11 w-full items-center gap-3 text-left disabled:opacity-50">
                                        <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 ${isSelected ? 'border-primary' : 'border-border'}`}>
                ...
                <div className="mx-auto max-w-lg px-4">
                    <div className="mb-3 flex items-center justify-between rounded-xl border border-border bg-surface-muted px-3 py-2">
                        ...
                        <div className="text-right">
                            <div className="text-xs font-semibold text-text-muted">Nilai Return</div>
                            <div className="text-sm font-bold text-primary">{formatCurrency(selectedSummary.totalValue)}</div>
                        </div>
                    </div>
                    <button
