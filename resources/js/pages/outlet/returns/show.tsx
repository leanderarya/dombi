            ...
            <div className="mt-4 pb-24">
                <div className="rounded-xl border border-border bg-white p-4">
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            <div className="text-sm font-bold text-text">Return #{ret.id}</div>
                            <div className="mt-1 text-xs text-text-muted">{formatDate(ret.created_at)}</div>
                        </div>
                ...
                <SectionCard label="Informasi" >
                    <dl className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                            ...
                            <dd className="font-bold tabular-nums text-text">{formatCurrency(ret.total_value)}</dd>
                        </div>
                        <div className="flex justify-between">
                        ...
                        {ret.review_notes && (
                            <div className="flex justify-between">
                                <dt className="text-text-muted">Catatan Owner</dt>
                                <dd className="text-text">{ret.review_notes}</dd>
                            </div>
                        )}
                    </dl>
                </SectionCard>
                        ...
                        <div className="space-y-3">
                            {ret.status_histories.map((h: any, i: number) => (
                                <div key={i} className="flex items-start gap-3">
                                    ...
                                    <div>
                                        <div className="text-sm font-medium text-text">{getReturnStatus(h.to_status).label}</div>
                ...
                {ret.exchange_request && (
                    ...
                    >
                        <div className="text-sm font-semibold text-text">Lihat Tukar Produk</div>
                        <div className="text-xs text-text-muted">#{ret.exchange_request.id}</div>
                    </Link>
                )}
            </div>
        </OutletLayout>
