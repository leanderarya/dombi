            ...
            <div className="mt-4 pb-24">
                <div className="rounded-xl border border-border bg-white p-4">
                    <div className="flex items-start justify-between gap-3">
                        <div>
                            <div className="text-sm font-bold text-text">Tukar Produk #{exchange.id}</div>
                            <div className="mt-1 text-xs text-text-muted">{formatDate(exchange.created_at)}</div>
                        </div>
                ...
                {exchange.return_request && (
                    <SectionCard label="Return Terkait" >
                        <Link href={`/outlet/returns/${exchange.return_request.id}`} className="block rounded-lg border border-border p-3 active:bg-surface-muted">
                            <div className="flex items-center justify-between">
                                <div className="text-sm font-semibold text-text">Return #{exchange.return_request.id}</div>
                                <StatusBadge status={exchange.return_request.status} />
                            </div>
                        </Link>
                    </SectionCard>
                )}
                ...
                <SectionCard label="Produk Pengganti" >
                    ...
                    <div className="mt-3 flex justify-between border-t border-border pt-3 text-sm">
                        ...
                        <span className="font-bold text-text">{formatCurrency(exchange.exchange_value)}</span>
                    </div>
                </SectionCard>
