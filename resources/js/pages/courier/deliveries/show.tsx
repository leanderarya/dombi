...
export default function CourierDeliveryShow({ delivery }: Props) {
    ...
    return (
        <CourierLayout
            ...
            actionBarSlot={
                hasActions ? (
                    <StickyActionBar actions={buildActions()} />
                ) : undefined
            }
        >
                        ...
                        {recipientDiffers ? (
                            <>
                                ...
                                <div className="mt-0.5 text-xs text-text-muted">
                                    Penerima · Pemesan: {order.customer_name}
                                </div>
                            </>
                        ) : (
                            <div className="text-sm font-semibold text-text">
            ...
            {delivery.status_histories &&
                delivery.status_histories.length > 0 &&
                (() => {
                            ...
                            <div className="border-t border-border px-4 pt-4 pb-4">
                                <div className="space-y-0">
                                    {timelineSteps.map((step, index) => {
                                        const history =
                                            delivery.status_histories.find(
                                                (h) => h.to_status === step.key,
                                            );
                                        ...
                                        return (
                                            ...
                                            >
                                                {!isLast && (
                                                    <div
                                                        className={`absolute top-6 bottom-0 left-[11px] w-px ${isCompleted ? 'bg-primary/20' : 'bg-border'}`}
                                                    />
                                                )}
                                                <div className="relative shrink-0 pt-0.5">
                                                    {isCompleted ? (
                                                ...
                                                <div className="min-w-0 flex-1 pt-0.5">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <div
                                                            className={`text-sm font-semibold ${isCurrent ? 'text-primary' : isCompleted ? 'text-text' : 'text-text-subtle'}`}
                                                        >
                                                            {step.label}
                                                        </div>
                                                        {history?.created_at && (
                                                            <span
                                                                className={`shrink-0 text-xs tabular-nums ${isCurrent ? 'font-semibold text-primary' : 'text-text-subtle'}`}
                                                            >
                                                                {formatTime(
                                                                    history.created_at,
                                                                )}
            ...
            {delivery.status === 'completed' && (
                <SectionCard
                    className="border-primary/20 bg-primary-light"
            ...
            <Dialog
                open={showFailSheet}
                onClose={() => setShowFailSheet(false)}
                title="Gagal Antar"
            >
                <FailSheetContent
                    deliveryId={delivery.id}
                ...
                {FAILURE_REASONS.map((reason) => (
                    ...
                    >
                        <input
                            ...
                            onChange={() =>
                                form.setData('failed_reason', reason)
                            }
                            className="mr-3"
                        />
                ...
                {REJECTION_REASONS.map((reason) => (
                    ...
                    >
                        <input
                            ...
                            onChange={() =>
                                form.setData('rejection_reason', reason)
                            }
                            className="mr-3"
                        />
                        <span className="text-sm font-medium text-text">
