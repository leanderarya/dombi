...
function findSmallestSize(variants: Variant[]): string | null {
    ...
    return sizes.length === 0
        ? null
        : sizes.reduce((a, b) => (sizeToMl(a) < sizeToMl(b) ? a : b));
}
...
function ProductDetailInner({
    ...
    const flavors = useMemo(
        () =>
            [
                ...new Set(
                    family.variants.map((v) => v.flavor).filter(Boolean),
                ),
            ] as string[],
        [family.variants],
    );
    const sortedSizes = useMemo(
        () =>
            (
                [
                    ...new Set(
                        family.variants.map((v) => v.size).filter(Boolean),
                    ),
                ] as string[]
            ).sort((a, b) => sizeToMl(a) - sizeToMl(b)),
        [family.variants],
    );
    const defaultFlavor = useMemo(
        () => findSoleFlavor(family.variants),
        [family.variants],
    );
    const defaultSize = useMemo(
        () => findSmallestSize(family.variants),
        [family.variants],
    );
                ...
                <main className="mx-auto max-w-lg px-4 pt-4 pb-32">
                    ...
                    {showFlavorSelector && (
                        <VariantSelector
                            ...
                            options={flavors.map((f) => ({
                                key: f,
                                label: f,
                                selected: effectiveFlavor === f,
                                hasVariant: family.variants.some(
                                    (v) =>
                                        v.flavor === f &&
                        ...
                        <VariantSelector
                            ...
                            options={sortedSizes.map((s) => {
                                ...
                                const basePrice =
                                    ...
                                    family.variants[0]?.selling_price ??
                                    0;
                    ...
                    {selectedVariant && (
                        <QuantitySelector
                            ...
                            max={effectiveMax}
                            lowStock={
                                selectedVariant.stock_status === 'low'
            ...
            <div className="mt-3 divide-y divide-border rounded-xl border border-border bg-white">
                {options.map((opt) => (
                    ...
                    >
                        ...
                        />
                        <span
                            className={`flex-1 text-sm ${opt.selected ? 'font-semibold text-text' : 'text-text'}`}
                        >
                            {opt.label}
                        </span>
                        {opt.right}
        ...
        <section className="mt-6">
            ...
            <div className="mt-3 flex items-center gap-4">
                ...
                <span className="text-sm text-text-muted">
                    Total <span className="font-bold text-text">{total}</span>
                </span>
            </div>
        </section>
    );
...
function OtherProducts({
    ...
    return (
        <section className="mt-8">
            ...
            <div className="-mx-4 mt-3 scrollbar-none flex gap-3 overflow-x-auto px-4 pb-2">
                {families.map((other) => {
                    const minPrice = Math.min(
                        ...(other.variants?.map((v) => v.selling_price) ?? [0]),
                    );
                    const href = outletId
