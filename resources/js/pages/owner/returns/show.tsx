...
export default function OwnerReturnsShow({ return: ret }: any) {
    ...
    if (!ret) {
        return (
            <OwnerPageShell title="Memuat..." subtitle="Detail return" backHref="/owner/returns">
                        ...
                        <div className="rounded-lg border border-border p-4">
                            ...
                            <div className="space-y-3">
                                {ret.status_histories.map((h: any, i: number) => {
                                    ...
                                    const histStatus = getReturnStatus(h.to_status);

                                    return (
                                        <div key={i} className="flex items-start gap-2.5">
                                            <div className="flex flex-col items-center">
                                                <div className={`h-2.5 w-2.5 shrink-0 rounded-full ${isLast ? 'bg-primary' : 'bg-border'}`} />
                                                {i < ret.status_histories.length - 1 && <div className="w-px h-4 bg-border mt-1" />}
                                            </div>
            ...
            <Dialog open={showReject} onOpenChange={setShowReject}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>Berikan alasan penolakan return ini.</DialogDescription>
                    </DialogHeader>
                    <Textarea
