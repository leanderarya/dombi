...
export default function OwnerExchangesShow({ exchange }: any) {
    ...
    if (!exchange) {
        return (
            <OwnerPageShell
                title="Memuat..."
                subtitle="Detail tukar produk"
                backHref="/owner/exchanges"
            >
            ...
            <div className="grid gap-4 lg:grid-cols-3">
                ...
                <div className="space-y-4 lg:col-span-2">
                    ...
                    {exchange.return_request && (
                        ...
                        >
                            ...
                            <OwnerDetailRow
                                label="Item"
                                value={`${exchange.return_request.items?.length ?? 0} item`}
                            />
                            <OwnerDetailRow
                                label="Status"
                                value={
                                    <StatusBadge
                                        variant={
                                            getReturnStatus(
                                                exchange.return_request.status,
                                            ).variant
                                        }
                                        size="sm"
                                    >
                ...
                <div className="space-y-4">
                    ...
                    >
                        ...
                        <OwnerDetailRow
                            label="Nilai Tukar"
                            value={formatCurrency(exchange.exchange_value)}
                            bold
                        />
                        <OwnerDetailRow
                            label="Item"
                            value={`${exchange.items?.length ?? 0} item`}
                        />
                        <OwnerDetailRow
                            label="Tanggal"
                            value={formatDate(exchange.created_at)}
                        />
                            ...
                            <div className="space-y-3">
                                {exchange.status_histories.map(
                                    (h: any, i: number) => {
                                        const isLast =
                                            i ===
                                            exchange.status_histories.length -
                                                1;
                                        const histStatus = getExchangeStatus(
                                            h.to_status,
                                        );
            ...
            <Dialog open={showReject} onOpenChange={setShowReject}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Berikan alasan penolakan tukar produk ini.
                        </DialogDescription>
                    </DialogHeader>
                    <Textarea
            ...
            <Dialog open={showComplete} onOpenChange={setShowComplete}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Tandai exchange ini selesai setelah outlet
                            mengonfirmasi produk pengganti diterima.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <Button
