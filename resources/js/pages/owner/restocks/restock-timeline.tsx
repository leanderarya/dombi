...
export function buildTimeline(restock: any): TimelineEvent[] {
    ...
    if (restock.received_at) {
        events.push({
            ...
            note:
                restock.received_notes || 'Outlet mengonfirmasi stok diterima.',
            active: true,
        });
    }
...
export default function RestockTimeline({
    events,
}: {
                ...
                return (
                    ...
                    >
                        <div className="flex flex-col items-center pt-0.5">
                            <div
                                className={`h-2 w-2 rounded-full ring-2 ring-offset-1 ${event.active ? 'bg-primary ring-primary/30' : 'bg-border ring-border/30'}`}
                            />
                            {!isLast && (
                                <div className="mt-1 min-h-6 w-px flex-1 bg-border/40" />
                            )}
                        </div>
                        <div className={`flex-1 ${isLast ? 'pb-0' : 'pb-4'}`}>
                            ...
                            {event.note && (
                                <div className="mt-1 rounded-md bg-surface-muted/50 px-2.5 py-1.5 text-xs leading-relaxed text-text-muted">
                                    {event.note}
                                </div>
                            )}
                        </div>
