...
export default function BreakdownCard({ title, data, statusLabels = {} }: Props) {
    ...
    return (
        <div className="rounded-lg border border-border bg-white" aria-label={title}>
            <button type="button" onClick={() => setOpen(!open)} className="flex w-full items-center justify-between p-3" aria-expanded={open}>
                <div className="text-xs font-medium text-text-subtle">{title}</div>
