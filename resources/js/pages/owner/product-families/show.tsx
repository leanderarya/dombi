...
// ── Constants ──
type SortKey = 'name' | 'center_price' | 'selling_price' | 'margin' | 'center_stock';
...
function sortVariants(variants: Variant[], key: SortKey, dir: 'asc' | 'desc'): Variant[] {
    return [...variants].sort((a, b) => {
        let av: string | number, bv: string | number;

        switch (key) {
            case 'name': av = a.name; bv = b.name; break;
            case 'center_price': av = a.center_price; bv = b.center_price; break;
            case 'selling_price': av = a.selling_price; bv = b.selling_price; break;
            case 'margin': av = margin(a); bv = margin(b); break;
            case 'center_stock': av = a.center_stock; bv = b.center_stock; break;
        }
...
export default function ProductFamilyShow({ family }: Props) {
    ...
    const bulkForm = useForm({ center_price: '', selling_price: '', center_stock: '', is_active: '' as '' | '1' | '0' });

    // Forms
    const familyForm = useForm({ name: family.name, brand: family.brand ?? '', description: family.description ?? '' });
    const variantForm = useForm({
        name: '', flavor: '', size: '', sku: '',
        center_price: '', selling_price: '', center_stock: '', is_active: true,
    });
    ...
    const filtered = useMemo(() => {
        ...
        switch (variantFilter) {
            case 'active': list = list.filter((v) => v.is_active); break;
            case 'inactive': list = list.filter((v) => !v.is_active); break;
            case 'low_margin': list = list.filter((v) => margin(v) >= 0 && margin(v) < 5000); break;
        }

        return list;
    }, [family.variants, search, variantFilter]);

    const sorted = useMemo(() => sortVariants(filtered, sortKey, sortDir), [filtered, sortKey, sortDir]);

    const maxMargin = useMemo(() => Math.max(...sorted.map((v) => margin(v)), 1), [sorted]);
    ...
    const toggleSelectAll = () => {
        if (selectedIds.size === sorted.length) { setSelectedIds(new Set()); } else { setSelectedIds(new Set(sorted.map((v) => v.id))); }
    };
    ...
    const handleUpdateVariant = (e: React.FormEvent) => {
        ...
        variantForm.put(`/owner/variants/${editingVariant.id}`, { onSuccess: () => { variantForm.reset(); setEditingVariant(null); } });
    };

    const startEdit = (v: Variant) => {
        setEditingVariant(v);
        variantForm.setData({ name: v.name, flavor: v.flavor ?? '', size: v.size ?? '', sku: v.sku ?? '', center_price: String(v.center_price), selling_price: String(v.selling_price), center_stock: String(v.center_stock), is_active: v.is_active });
    };

    const duplicate = (v: Variant) => {
        setEditingVariant(null);
        variantForm.setData({ name: '', flavor: v.flavor ?? '', size: v.size ?? '', sku: '', center_price: String(v.center_price), selling_price: String(v.selling_price), center_stock: String(v.center_stock), is_active: true });
    ...
    if (family === undefined || family === null) {
        return <OwnerPageShell title="Product Family" subtitle="Memuat..."><SkeletonPage /></OwnerPageShell>;
    }
        ...
        <OwnerPageShell
            ...
            headerRight={
                <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={() => setShowFamilyEdit(true)}><Pencil className="h-4 w-4 mr-1" aria-hidden="true" />Edit</Button>
                    <Button variant="danger" onClick={() => setDeleteFamilyDialog(true)}><Trash2 className="h-4 w-4 mr-1" aria-hidden="true" />Hapus</Button>
                </div>
            }
        >
            {family.description && (
            ...
            <OwnerFilterCard
                ...
                tambahOnClick={() => { cancelForm(); setShowVariantForm(true); }}
            />
            ...
            {/* Empty State */}
            {sorted.length === 0 ? (
                <EmptyState icon={<Package className="h-8 w-8 text-slate-400" />} title={family.variants.length === 0 ? 'Belum ada variant' : 'Tidak ditemukan'} description={family.variants.length === 0 ? 'Tambah variant pertama' : 'Coba kata kunci lain'} action={family.variants.length === 0 ? { label: 'Tambah Variant', onClick: () => setShowVariantForm(true) } : undefined} />
            ) : (
                <>
                    {/* Bulk Edit Bar */}
                    {selectedIds.size > 0 && (
                        <div className="mb-3 flex items-center gap-3 rounded-lg border border-blue-200 bg-blue-50 px-4 py-2">
                            <span className="text-sm font-medium text-blue-700">{selectedIds.size} dipilih</span>
                            <Button type="button" variant="primary" size="sm" onClick={() => setBulkModalOpen(true)}>Edit Massal</Button>
                            <Button type="button" variant="link" size="sm" onClick={() => setSelectedIds(new Set())}>Batal</Button>
                        </div>
                    )}
                            ...
                            <thead>
                                <tr className="border-b border-border bg-surface-muted/50 text-left">
                                    <th className="w-8 px-3 py-2.5">
                                        <input type="checkbox" checked={selectedIds.size === sorted.length && sorted.length > 0} onChange={toggleSelectAll} className="h-3.5 w-3.5 rounded border-zinc-300 accent-primary" />
                                    </th>
                                    ...
                                    <th className="cursor-pointer select-none px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wide text-text-muted" onClick={() => toggleSort('center_stock')}>
                                        Stok<SortMarker col="center_stock" />
                                    </th>
                                    <th className="w-28 px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-wide text-text-muted">
                                        Aksi
                                    </th>
                                </tr>
                            </thead>
                                        ...
                                        <tr key={v.id} className={`transition-colors hover:bg-mint-wash/30 ${!v.is_active ? 'opacity-50' : ''} ${isSelected ? 'bg-blue-50/50' : ''}`}>
                                            ...
                                            <td className="px-3 py-3">
                                                ...
                                                <div className="mt-0.5 flex items-center gap-1.5">
                                                    {v.sku && <span className="text-xs text-text-subtle">{v.sku}</span>}
                                                    {!v.is_active && <StatusBadge variant="neutral" size="sm">Nonaktif</StatusBadge>}
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-3 py-3 text-right">
                                                <MarginBarInline margin={m} maxMargin={maxMargin} sellingPrice={v.selling_price} />
                                            </td>
            ...
            <Dialog open={showFamilyEdit} onOpenChange={setShowFamilyEdit}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Edit Product Family</DialogTitle><DialogDescription>Perbarui data product family.</DialogDescription></DialogHeader>
                    <form onSubmit={handleUpdateFamily}>
                        <div className="space-y-3">
                            <Input label="Nama" type="text" value={familyForm.data.name} onChange={(e) => familyForm.setData('name', e.target.value)} required error={familyForm.errors.name} />
                            <Input label="Brand" type="text" value={familyForm.data.brand} onChange={(e) => familyForm.setData('brand', e.target.value)} />
                            <Textarea label="Deskripsi" value={familyForm.data.description} onChange={(e) => familyForm.setData('description', e.target.value)} rows={2} />
                        </div>
                    </form>
                    <DialogFooter>
            ...
            <Dialog open={bulkModalOpen} onOpenChange={setBulkModalOpen}>
                <DialogContent className="sm:max-w-md">
                    <DialogHeader><DialogTitle>Edit Massal ({selectedIds.size} variant)</DialogTitle><DialogDescription>Hanya field yang diisi yang akan diperbarui.</DialogDescription></DialogHeader>
                    <form onSubmit={(e) => {
                        e.preventDefault();
                        const payload: { variant_ids: number[] } & Record<string, string | number | boolean> = { variant_ids: [...selectedIds] } as { variant_ids: number[] } & Record<string, string | number | boolean>;
                        ...
                        bulkForm.post(`/owner/product-families/${family.id}/variants/bulk-update`, { onSuccess: () => { bulkForm.reset(); setBulkModalOpen(false); setSelectedIds(new Set()); } });
                    }}>
                        <div className="space-y-3">
                            <Input label="HPP (Rp)" type="number" value={bulkForm.data.center_price} onChange={(e) => bulkForm.setData('center_price', e.target.value)} placeholder="Kosongkan jika tidak diubah" />
                            <Input label="Harga Jual (Rp)" type="number" value={bulkForm.data.selling_price} onChange={(e) => bulkForm.setData('selling_price', e.target.value)} placeholder="Kosongkan jika tidak diubah" />
                            <Input label="Stok" type="number" value={bulkForm.data.center_stock} onChange={(e) => bulkForm.setData('center_stock', e.target.value)} placeholder="Kosongkan jika tidak diubah" />
                            <select value={bulkForm.data.is_active} onChange={(e) => bulkForm.setData('is_active', e.target.value as '' | '1' | '0')} className="h-8 w-full rounded-md border border-border bg-surface px-2 text-xs">
                        ...
                        <DialogFooter>
                            <Button type="button" variant="outline" onClick={() => { setBulkModalOpen(false); bulkForm.reset(); }}>Batal</Button>
                            <Button type="submit" disabled={bulkForm.processing}>{bulkForm.processing ? 'Menyimpan...' : 'Simpan'}</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Hapus Variant</DialogTitle><DialogDescription>Yakin ingin menghapus variant ini?</DialogDescription></DialogHeader>
                    <DialogFooter><Button variant="outline" onClick={() => setDeleteId(null)}>Batal</Button><Button variant="destructive" onClick={handleDeleteVariant}>Hapus</Button></DialogFooter>
                </DialogContent>
            </Dialog>

            <Dialog open={deleteFamilyDialog} onOpenChange={() => setDeleteFamilyDialog(false)}>
                <DialogContent>
                    <DialogHeader><DialogTitle>Hapus Product Family</DialogTitle><DialogDescription>Semua variant juga akan dihapus.</DialogDescription></DialogHeader>
                    <DialogFooter><Button variant="outline" onClick={() => setDeleteFamilyDialog(false)}>Batal</Button><Button variant="destructive" onClick={handleDeleteFamily}>Hapus</Button></DialogFooter>
                </DialogContent>
            </Dialog>
        </OwnerPageShell>
    );
