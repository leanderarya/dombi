...
function buildProductGroups(outletSections: any[]) {
    ...
    for (const section of outletSections ?? []) {
        for (const item of section.inventories ?? []) {
            ...
            const enriched = { ...item, outlet_name: section.outlet.name, outlet_id: section.outlet.id };
...
export default function InventoriesIndex({ tab: initialTab, outletSections, stats, centralStock, centralStats }: any) {
    ...
    const toggleSubSort = (key: SubSortKey) => {
        if (subSortKey === key) { setSubSortDir((d) => (d === 'asc' ? 'desc' : 'asc')); } else { setSubSortKey(key); setSubSortDir('asc'); }
    };
    ...
    if (!outletSections && !centralStock) {
        return <OwnerPageShell title="Inventaris" subtitle="Pantau stok semua outlet dan pusat"><SkeletonPage /></OwnerPageShell>;
    }

    const outletList = useMemo(() => (outletSections ?? []).map((s: any) => ({ id: s.outlet.id, name: s.outlet.name })), [outletSections]);

    const productGroups = useMemo(() => buildProductGroups(outletSections), [outletSections]);
    ...
    const filtered = useMemo(() => {
        ...
        if (search) {
            ...
            result = result.filter((g) => {
                ...
                return name.includes(q) || (g.variant?.sku ?? '').toLowerCase().includes(q) ||
                    g.outlets.some((o: any) => o.outlet_name.toLowerCase().includes(q));
            });
        }

        if (outletFilter !== 'all') {
            result = result.filter((g) => g.outlets.some((o: any) => o.outlet_name === outletFilter));
        }
    ...
    const sorted = useMemo(() => [...filtered].sort((a, b) => {
        let av: any, bv: any;
        const statusToNum = (s: string) => s === 'critical' ? 0 : s === 'low' ? 1 : 2;

        switch (sortKey) {
            case 'name': av = displayProductName(a.variant); bv = displayProductName(b.variant); break;
            case 'total_stock': av = a.totalStock; bv = b.totalStock; break;
            case 'status': av = statusToNum(a.overallStatus); bv = statusToNum(b.overallStatus); break;
            default: av = displayProductName(a.variant); bv = displayProductName(b.variant);
        }
            ...
            <div className="mb-5 inline-flex rounded-lg bg-surface-muted p-1" role="tablist">
                {TABS.map((t) => (
                    <button key={t.key} type="button" role="tab" aria-selected={activeTab === t.key} onClick={() => handleTabChange(t.key)}
                        className={cn('relative rounded-lg px-5 py-2 text-sm font-semibold transition-all duration-200', activeTab === t.key ? 'bg-surface text-text' : 'text-text-muted hover:text-text')}>
                        {t.label}
                    </button>
                ))}
            </div>
            ...
            {activeTab === 'outlet' && (
                <>
                    <OwnerKpiStrip cols={4} items={[
                        ...
                        { label: 'Stok Kritis', value: stats.critical, sublabel: stats.critical > 0 ? '≤ 2 pcs' : undefined, sublabelColor: 'text-red-500' },
                        { label: 'Stok Rendah', value: stats.lowStock, sublabel: stats.lowStock > 0 ? '≤ minimum' : undefined, sublabelColor: 'text-amber-500' },
                        { label: 'Stok Sehat', value: (stats.totalSku - stats.lowStock) },
                    ]} />
                    ...
                    <OwnerFilterCard collapsible defaultExpanded={false} searchPlaceholder="Cari produk atau outlet..." searchValue={search} onSearch={setSearch}>
                        <select value={outletFilter} onChange={(e) => setOutletFilter(e.target.value)}
                            ...
                            {outletList.map((o: any) => <option key={o.id} value={o.name}>{o.name}</option>)}
                        </select>
                    </OwnerFilterCard>
                    ...
                    {sorted.length === 0 ? (
                        ...
                        <div className="overflow-x-auto rounded-xl bg-surface shadow-card">
                            <table className="w-full min-w-[700px]" aria-label="Stok Outlet — Grup per Produk">
                                <thead>
                                    <tr className="bg-surface-muted/50">
                                        <th className="w-8 px-3 py-2.5" />
                                        <SortableTh label="Produk" active={sortKey === 'name'} dir={sortDir} onClick={() => toggleSort('name')} />
                                        <th className="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-text-muted">Status Outlet</th>
                                        <SortableTh label="Total Stok" active={sortKey === 'total_stock'} dir={sortDir} align="right" onClick={() => toggleSort('total_stock')} />
                                        <SortableTh label="Status" active={sortKey === 'status'} dir={sortDir} onClick={() => toggleSort('status')} />
                                    </tr>
                                </thead>
                                <tbody>
                                    {sorted.map((group) => {
                                        const isExpanded = expandedIds.has(group.variantId);
                                        const productName = displayProductName(group.variant);
                                        const statusVariant = group.overallStatus === 'critical' ? 'danger' : group.overallStatus === 'low' ? 'warning' : 'success';
                                        const statusLabel = group.overallStatus === 'critical' ? 'Kritis' : group.overallStatus === 'low' ? 'Rendah' : 'Sehat';

                                        const sortedOutlets = [...group.outlets].sort((a: any, b: any) => {
                                            let av: any, bv: any;
                                            switch (subSortKey) {
                                                case 'outlet_name': av = a.outlet_name; bv = b.outlet_name; break;
                                                ...
                                                case 'status': av = a.current_stock <= 2 ? 0 : a.current_stock <= (a.minimum_stock ?? 0) ? 1 : 2; bv = b.current_stock <= 2 ? 0 : b.current_stock <= (b.minimum_stock ?? 0) ? 1 : 2; break;
                                                default: av = a.outlet_name; bv = b.outlet_name;
                                            }
                                            const cmp = typeof av === 'string' ? av.localeCompare(String(bv)) : Number(av) - Number(bv);
                                        ...
                                        return (
                                            <>
                                                <tr key={group.variantId} className="border-t border-border/20 transition-colors hover:bg-mint-wash cursor-pointer"
                                                    onClick={() => toggleExpand(group.variantId)}>
                                                    <td className="px-3 py-3 text-center">
                                                        {isExpanded ? <ChevronDown className="mx-auto h-4 w-4 text-text-muted" /> : <ChevronRight className="mx-auto h-4 w-4 text-text-muted" />}
                                                    </td>
                                                    <td className="px-3 py-3">
                                                        <span className="font-bold text-text">{productName}</span>
                                                        {group.variant?.sku && <span className="ml-1 text-xs text-text-muted">{group.variant.sku}</span>}
                                                    </td>
                                                    <td className="px-3 py-3">
                                                        <div className="flex items-center gap-1.5">
                                                            <div className="flex gap-0.5">
                                                                {group.outlets.map((o: any) => {
                                                                    const s = o.current_stock <= 2 ? 'critical' : o.current_stock <= (o.minimum_stock ?? 0) ? 'low' : 'healthy';
                                                                    return (
                                                                        <span key={o.id} className={cn('h-2.5 w-2.5 rounded-full',
                                                                            s === 'critical' ? 'bg-red-500' : s === 'low' ? 'bg-amber-400' : 'bg-emerald-500'
                                                                        )} title={`${o.outlet_name}: ${o.current_stock} pcs`} />
                                                                    );
                                                                })}
                                                            </div>
                                                            <span className="text-xs text-text-muted">
                                                                {group.outlets.length} outlet{group.criticalCount > 0 && ` · ${group.criticalCount} kritis`}{group.lowCount > 0 && ` · ${group.lowCount} rendah`}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className={cn('px-3 py-3 text-right font-bold tabular-nums', group.overallStatus === 'critical' ? 'text-red-600' : group.overallStatus === 'low' ? 'text-amber-600' : 'text-emerald-600')}>
                                                ...
                                                {isExpanded && (
                                                    <tr key={`${group.variantId}-sub`} className="border-t border-border/50 bg-surface-muted/30">
                                                        <td colSpan={5} className="px-0 py-0">
                                                            <div className="overflow-x-auto">
                                                                <table className="w-full min-w-[600px]" aria-label={`Detail stok ${productName} per outlet`}>
                                                                    <thead>
                                                                        <tr className="border-b border-border/50 bg-surface-muted/50 text-xs">
                                                                            <th className="w-8 px-3 py-2" />
                                                                            <SortableTh label="Outlet" active={subSortKey === 'outlet_name'} dir={subSortDir} onClick={() => toggleSubSort('outlet_name')} />
                                                                            <SortableTh label="Stok" active={subSortKey === 'current_stock'} dir={subSortDir} align="right" onClick={() => toggleSubSort('current_stock')} />
                                                                            <SortableTh label="Min" active={subSortKey === 'minimum_stock'} dir={subSortDir} align="right" onClick={() => toggleSubSort('minimum_stock')} />
                                                                            <SortableTh label="Status" active={subSortKey === 'status'} dir={subSortDir} onClick={() => toggleSubSort('status')} />
                                                                            <th className="w-40 px-3 py-2 text-right">Aksi</th>
                                                                        </tr>
                                                                    </thead>
                                                                    <tbody>
                                                                        {sortedOutlets.map((row: any) => {
                                                                            const isCritical = row.current_stock <= 2;
                                                                            const isLow = !isCritical && row.current_stock <= (row.minimum_stock ?? 0);
                                                                            const variantName = displayProductName(row.variant);
                                                                            const remindKey = `${row.outlet_id}-${(row.product_variant_id ?? row.variant?.id)}`;
                                                                            const reminded = remindedIds.has(remindKey);

                                                                            return (
                                                                                <tr key={row.id} className="border-t border-border/30 transition-colors hover:bg-mint-wash">
                                                                                    <td className="px-3 py-2.5" />
                                                                                    <td className="px-3 py-2.5 text-sm font-medium text-text">{row.outlet_name}</td>
                                                                                    <td className={cn('px-3 py-2.5 text-right font-bold tabular-nums', isCritical ? 'text-red-600' : isLow ? 'text-amber-600' : 'text-emerald-600')}>
                                                                                        {row.current_stock}
                                                                                    </td>
                                                                                    <td className="px-3 py-2.5 text-right tabular-nums text-text-muted">{row.minimum_stock}</td>
                                                                                    <td className="px-3 py-2.5">
                                                                                        ...
                                                                                        <div className="flex items-center justify-end gap-1">
                                                                                            {(isCritical || isLow) && (
                                                                                                reminded ? (
                                                                                                    ...
                                                                                                    <Button size="sm" variant="secondary" onClick={(e) => { e.stopPropagation(); handleRemind(row, variantName, isCritical); }}>
                                                                                                        <Bell className="mr-1 h-3 w-3" />Ingatkan
                                                                                                    </Button>
                                                                                                )
                                                                                            )}
                                                                                            <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); setEditItem(row); editForm.setData({ current_stock: row.current_stock, minimum_stock: row.minimum_stock, notes: '' }); }}>Edit</Button>
                                                                                        </div>
                                                                                    </td>
                                                                                </tr>
                                                                            );
            ...
            <Dialog open={editItem !== null} onOpenChange={() => setEditItem(null)}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {displayProductName(editItem?.variant)} — {editItem?.outlet_name}
                        </DialogDescription>
                    </DialogHeader>
