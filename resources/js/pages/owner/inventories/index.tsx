...
export default function InventoriesIndex({ inventories, outlets, products, filters }: any) {
    return <OwnerLayout><Head title="Inventory" /><div className="flex items-center justify-between"><h1 className="text-2xl font-semibold">Inventory</h1><Link href="/owner/inventories/create" className="rounded-md bg-emerald-700 px-4 py-2 text-white">Tambah Stok</Link></div>
        ...
        <div className="mt-5 overflow-x-auto rounded-lg border border-zinc-200 bg-white"><table className="w-full text-left text-sm"><thead className="bg-zinc-50"><tr><th className="p-3">Outlet</th><th>Produk</th><th>Current</th><th>Reserved</th><th>Available</th><th>Minimum</th><th>Level</th><th></th></tr></thead><tbody>{inventories.data.map((item: any) => <tr key={item.id} className="border-t"><td className="p-3">{item.outlet.name}</td><td>{item.product.name}</td><td>{item.current_stock}</td><td>{item.reserved_stock}</td><td className="font-medium">{item.current_stock - item.reserved_stock}</td><td>{item.minimum_stock}</td><td><StockLevelBadge currentStock={item.current_stock} reservedStock={item.reserved_stock} minimumStock={item.minimum_stock} /></td><td className="p-3 text-right"><Link href={`/owner/inventories/${item.id}/edit`} className="text-emerald-700">Edit</Link></td></tr>)}</tbody></table></div>
    </OwnerLayout>;
}
