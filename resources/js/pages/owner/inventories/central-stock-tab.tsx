...
export default function CentralStockTab({ variants, stats }: { variants?: any[]; stats?: any }) {
    ...
    const filtered = useMemo(() => {
        ...
        if (search) {
            ...
            list = list.filter((v: any) => v.name.toLowerCase().includes(q) || (v.sku ?? '').toLowerCase().includes(q));
        }

        switch (stockFilter) {
            case 'empty': list = list.filter((v: any) => v.center_stock <= 0); break;
            case 'low': list = list.filter((v: any) => v.center_stock > 0 && v.center_stock <= 10); break;
            case 'healthy': list = list.filter((v: any) => v.center_stock > 10); break;
        }
        ...
        <>
                    ...
                    <table className="w-full min-w-[600px]" aria-label="Stok Pusat">
                        <thead>
                            <tr className="bg-surface-muted">
                                ...
                                <th className="w-20 px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-wide text-text-muted">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            {sorted.map((v: any) => {
                                ...
                                return (
                                    <tr key={v.id} className="border-t border-border transition-colors hover:bg-surface-muted">
                                        ...
                                        <td className="px-3 py-3 text-right">
                                            <Button variant="ghost" size="sm" onClick={() => { setEditModal(v); setNewStock(String(v.center_stock)); setReason(''); }}>Edit</Button>
                                        </td>
                                    </tr>
                                );
                            })}
            ...
            <Dialog open={!!editModal} onOpenChange={(isOpen) => { if (!isOpen) setEditModal(null); }}>
                <DialogContent className="max-w-sm">
                    <DialogHeader><DialogTitle>Edit Stok Pusat</DialogTitle><DialogDescription>{editModal?.name}</DialogDescription></DialogHeader>
                    <div className="space-y-3">
                        <div>
                            <label className="mb-1 block text-xs font-medium text-text-muted">Stok Saat Ini</label>
                            <div className="rounded-lg border border-border bg-surface-muted px-3 py-2 text-sm text-text-muted">{editModal?.center_stock} pcs</div>
                        </div>
                        <Input label="Stok Baru" type="number" value={newStock} onChange={(e) => setNewStock(e.target.value)} min={0} autoFocus />
                        <Select label="Alasan" value={reason} onChange={(e) => setReason(e.target.value)} options={reasonOptions} placeholder="Pilih alasan..." />
                    </div>
                    <DialogFooter>
                        ...
                        <Button onClick={() => {
                            if (!editModal) return;
                            setSaving(true);
                            router.patch(`/owner/inventories/central-stock/${editModal.id}`, { center_stock: parseInt(newStock), reason: reason || undefined }, { onFinish: () => { setSaving(false); setEditModal(null); } });
                        }} disabled={saving || parseInt(newStock) === editModal?.center_stock} loading={saving}>Simpan</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
