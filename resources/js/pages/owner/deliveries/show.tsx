        ...
        >
            <div className="grid gap-3 lg:grid-cols-2">
                ...
                <div className="rounded-lg border border-border p-4">
                    ...
                    {isActive && (
                        <div className="mt-3 flex h-8 items-center justify-center gap-2 rounded-lg bg-indigo-50 text-xs font-semibold text-indigo-700">
                ...
                {/* Delivery Info */}
                <div className="rounded-lg border border-border p-4">
                    <div className="mb-3 text-[11px] font-bold uppercase tracking-wide text-text-subtle">Info Pengiriman</div>
                    <div className="flex justify-between border-b border-[#f5f5f5] py-1 text-xs last:border-b-0">
                ...
                <div className="rounded-lg border border-border p-4 lg:col-span-2">
                    <div className="mb-3 text-[11px] font-bold uppercase tracking-wide text-text-subtle">Item Pesanan</div>
                    ...
                    <div className="space-y-2">
                        {order.status_histories.map((history: any) => (
                            <div key={history.id} className="border-l-2 border-primary/20 pl-3">
                                <div className="text-xs font-medium text-text">{history.to_status.replaceAll('_', ' ')}</div>
                                {history.notes && <div className="text-[11px] text-text-muted">{history.notes}</div>}
                ...
                {canResolve && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 lg:col-span-2">
                        <div className="mb-1 text-xs font-bold text-amber-900">Insiden Belum Diselesaikan</div>
                        <p className="text-[11px] text-amber-700">
                            Delivery ini gagal dan membutuhkan tindakan operasional.
                        </p>
                        <button
                            onClick={() => setResolveOpen(true)}
                            className="mt-3 flex h-9 w-full items-center justify-center gap-2 rounded-lg bg-amber-600 text-xs font-semibold text-white transition-colors hover:bg-amber-700 active:bg-amber-800"
                        >
                            Selesaikan Insiden
                        </button>
                    </div>
