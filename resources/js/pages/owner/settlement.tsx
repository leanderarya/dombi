...
export default function OwnerSettlement({ dashboard, reconciliation, overdue, period }: Props) {
    ...
    return (
        <>
            <Head title="Settlement Dashboard" />

            <div className="p-4">
                <h1 className="mb-4 text-lg font-bold text-slate-900">Settlement Dashboard</h1>
                ...
                <div className="mb-4 grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-zinc-200 bg-white p-4">
                        <div className="text-xs text-zinc-500">Total Center Share</div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-zinc-200">
                                    ...
                                    <th className="pb-2 text-right font-medium text-zinc-500">Center Share</th>
                                    <th className="pb-2 text-right font-medium text-zinc-500">Dibayar</th>
                                    <th className="pb-2 text-right font-medium text-zinc-500">Outstanding</th>
                        ...
                        <div className="space-y-2">
                            {reconciliation.recent_payments.map((p) => (
                                <div key={p.id} className="flex items-center justify-between rounded-lg bg-zinc-50 p-3">
                                    <div>
                                        <div className="text-sm font-medium text-slate-900">{p.outlet.name}</div>
                                        <div className="text-xs text-zinc-500">{p.reference_number} • {formatDate(p.payment_date)}</div>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-sm font-bold text-emerald-700">{formatCurrency(p.amount)}</div>
