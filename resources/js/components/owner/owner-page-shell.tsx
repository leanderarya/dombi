...
export default function OwnerPageShell({ title, subtitle, backHref, headerRight, children }: Props) {
    return (
        <OwnerLayout>
            ...
            <PageHeader title={title} subtitle={subtitle} backHref={backHref} headerRight={headerRight} />
            {children}
            </div>
        </OwnerLayout>
    );
...
function PageHeader({ title, subtitle, backHref, headerRight }: Omit<Props, 'children'>) {
