    ...
    return (
        ...
        >
            {items.map((item, i) => {
                ...
                const trendPositive =
                    item.trend?.positive ?? item.trend?.direction === 'up';

                const content = (
                    <div
                        className={`group hover:shadow-card-hover flex flex-col justify-between rounded-xl bg-surface p-6 shadow-card transition-all duration-300 hover:-translate-y-1 ${spanClass}`}
                    >
                        ...
                        <div className="mt-6">
                            ...
                            <div
                                className={`mt-1.5 text-2xl font-bold tracking-tight tabular-nums ${item.valueClassName ?? 'text-text'}`}
                            >
                                {item.value}
                            </div>
                        </div>
                        <div className="mt-4 flex h-5 items-center gap-1.5">
                            ...
                            {!item.trend && item.sublabel && (
                                <span
                                    className={`text-xs ${item.sublabelColor ?? 'text-text-muted'}`}
                                >
                                    {item.sublabel}
                                </span>
                            )}
