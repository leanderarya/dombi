...
export default function MarginBar({ margin, maxMargin, sellingPrice = 0, className }: Props) {
    if (sellingPrice <= 0 && margin <= 0) {
        return null;
    }

    const pct = maxMargin > 0 ? Math.min(Math.abs(margin) / maxMargin, 1) * 100 : 0;
    ...
    return (
        <div className={cn('h-1.5 w-16 overflow-hidden rounded-full bg-slate-100', className)}>
