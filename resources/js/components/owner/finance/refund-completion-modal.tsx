        ...
        <Dialog open={open} onOpenChange={(v) => !v && !busy && onClose()}>
                <DialogContent>
                    <DialogHeader className="space-y-1">
                        <DialogTitle id="complete-dialog-title" className="text-lg">Selesaikan Refund</DialogTitle>
                        <DialogDescription id="complete-dialog-desc" className="flex flex-wrap items-center gap-1.5">
                            <span className="font-medium text-text">#{orderCode}</span>
                    ...
                    <form onSubmit={submit} aria-labelledby="complete-dialog-title" className="space-y-4">
                        <div>
                            ...
                            {preview && (
                                <div className="mt-2">
                                    <img src={preview} alt="Preview bukti transfer" className="max-h-36 w-full rounded-lg border border-border object-contain bg-muted" />
                                </div>
                            )}
                        </div>
                        <div>
                            <Label htmlFor="reference">Referensi Transfer</Label>
                            <Input id="reference" value={reference} onChange={(e) => setReference(e.target.value)} placeholder="B1234567890" maxLength={255} className="min-h-11" />
                        </div>
                        <div>
                            <Label htmlFor="note">Catatan (opsional)</Label>
                            <Textarea id="note" value={note} onChange={(e) => setNote(e.target.value)} placeholder="..." rows={2} maxLength={500} />
                            <p className="mt-0.5 text-[11px] text-text-subtle">{note.length}/500</p>
                        </div>
