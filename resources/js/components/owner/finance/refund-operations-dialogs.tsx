...
export function GuestRefundDestinationDialog({
    refund,
    open,
    onClose,
}: DialogProps) {
    ...
    const submit: FormEventHandler = (e) => {
        ...
        setBusy(true);
        router.post(
            `/owner/refunds/${refund.order_id}/destination`,
            {
    ...
    return (
        <Dialog open={open} onOpenChange={(v) => !v && !busy && closeDialog()}>
            <DialogContent>
                ...
                <form
                    onSubmit={submit}
                    aria-labelledby="guest-dest-title"
                    className="space-y-4"
                >
                    {refund.destination && (
                        <div className="space-y-1 rounded-lg border border-border bg-muted p-3 text-xs">
                            ...
                            <p className="break-all">
                                {refund.destination.label} —{' '}
                                {refund.destination.holder}
                            </p>
                    ...
                    <p className="text-xs text-text-muted">
                        Gunakan hanya untuk proses refund order ini.
                    </p>
                    <div>
                    ...
                    {isBank ? (
                        <>
                            <div>
                                <Label htmlFor="gb_bank">Nama Bank</Label>
                                <Input
                                    id="gb_bank"
                                    value={bankName}
                                    onChange={(e) =>
                            ...
                            <div>
                                <Label htmlFor="gb_holder">Nama Pemilik</Label>
                                <Input
                                    id="gb_holder"
                                    value={accountHolder}
                                    onChange={(e) =>
                        ...
                        <>
                            ...
                            <div>
                                <Label htmlFor="ge_holder">Nama Pemilik</Label>
                                <Input
                                    id="ge_holder"
                                    value={ewalletHolder}
                                    onChange={(e) =>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Label htmlFor="phone_verified" className="text-xs">
                            Saya sudah memverifikasi tujuan refund melalui nomor
                            pesanan
                        </Label>
                    </div>
...
export function RefundRollbackDialog({ refund, open, onClose }: DialogProps) {
    ...
    const submit: FormEventHandler = (e) => {
        ...
        setBusy(true);
        router.post(
            `/owner/refunds/${refund.order_id}/rollback`,
            {
                mode,
                reason: reason.trim(),
            },
                ...
                >
                    <div role="radiogroup" aria-label="Mode rollback">
                        ...
                        <div className="mt-1 space-y-1">
                            <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-border p-2 text-xs has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                <input
                                    type="radio"
                                    name="mode"
                                    value="retry"
                                ...
                                <div>
                                    ...
                                    — tujuan refund valid, ulangi proses
                                </div>
                            </label>
                            <label className="flex cursor-pointer items-center gap-2 rounded-lg border border-border p-2 text-xs has-[:checked]:border-primary has-[:checked]:bg-primary/5">
                                <input
                                ...
                                <div>
                                    ...
                                    — tujuan refund tidak valid, minta data baru
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <Button
                            ...
                            className="min-h-11"
                        >
                            {busy ? 'Memproses...' : 'Kembalikan ke Antrean'}
                        </Button>
                    </div>
                </form>
            </DialogContent>
