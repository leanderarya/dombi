...
export default function DeliveryTimeline({ histories }: Props) {
    if (histories.length === 0) {
        return (
            <div className="py-4 text-center text-xs text-slate-400">Belum ada riwayat</div>
        );
    }
    ...
    return (
        <div className="space-y-0">
            {histories.map((h, i) => {
                const color = statusColors[h.to_status] ?? 'border-slate-300 bg-slate-100';
                ...
                return (
                    <div key={h.id} className="flex gap-3">
                        ...
                        <div className="flex flex-col items-center">
                            <div className={`flex h-6 w-6 items-center justify-center rounded-full border-2 ${color}`}>
