...
const STATUS_GUIDANCE: Record<string, { description: string; nextStep?: string; cta?: { label: string; href?: string; action?: string } }> = {
    ...
    pending_confirmation_payment_failed: { description: 'Pembayaran Gagal', nextStep: 'Pembayaran sebelumnya gagal. Anda masih bisa coba bayar ulang sebelum waktu habis.' },
...
export default function StatusGuidanceCard({ status, paymentStatus, isPickup, confirmationExpiresAt, outletPhone, outletLatitude, outletLongitude }: Props) {
    const countdown = useCountdown(confirmationExpiresAt);
    const isPendingUnpaid = status === 'pending_confirmation' && paymentStatus !== 'paid';
    ...
    const guidanceKey = isPendingUnpaid
        ...
        : (status === 'ready_for_pickup' && isDelivery ? 'ready_for_pickup_delivery' : status);
    const guidance = STATUS_GUIDANCE[guidanceKey];

    if (!guidance) return null;

    const isPickupReady = status === 'ready_for_pickup' && outletLatitude && outletLongitude;
    const showCountdown = status === 'pending_confirmation' && !isPendingUnpaid && !countdown.expired && countdown.totalSeconds > 0;
                ...
                {guidance.cta && (
                    <div className="shrink-0">
                        {isPickupReady && guidance.cta.action === 'navigate' ? (
                            <a href={`${MAPS_LINK}${outletLatitude},${outletLongitude}`} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-3 text-xs font-bold text-white active:opacity-80">
                                <MapPin className="h-3.5 w-3.5" />{guidance.cta.label}
                            </a>
                        ) : guidance.cta.action === 'wa_outlet' && outletPhone ? (
                            <a href={`${WA_LINK}${outletPhone.replace(/^0/, '62')}`} target="_blank" rel="noopener noreferrer" className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-3 text-xs font-bold text-white active:opacity-80">
