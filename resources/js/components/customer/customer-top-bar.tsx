        ...
        <header className="sticky top-0 z-30 border-b border-border bg-white/95 backdrop-blur pt-[env(safe-area-inset-top)]">
            <div className="mx-auto flex max-w-lg items-center justify-between gap-3 px-4 py-3">
                <button type="button" onClick={() => setSheetOpen(true)} className="min-w-0 flex-1 text-left active:opacity-80">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-text-subtle">Kirim ke</div>
                    <div className="mt-0.5 flex items-center gap-1">
                        <span className="truncate text-sm font-semibold text-text">
                            {addressText || 'Tentukan Lokasi Anda'}
                        </span>
                        <svg className="h-3.5 w-3.5 shrink-0 text-text-subtle" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                ...
                <div className="flex items-center gap-1">
                    ...
                    {isLoggedIn ? (
                        <Link href="/customer/profile" className="flex h-10 w-10 items-center justify-center rounded-lg text-text-muted active:bg-zinc-100">
                            {auth.user.avatar ? (
                    ...
                    {/* Cart */}
                    <Link href="/customer/checkout" className="relative flex h-10 w-10 items-center justify-center rounded-lg text-text-muted active:bg-zinc-100">
                        <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 100 4 2 2 0 000-4z" />
                        </svg>
                        {totalItems > 0 && (
                            <span className="absolute -right-0.5 -top-0.5 flex h-4.5 min-w-4.5 items-center justify-center rounded-full bg-emerald-600 px-1 text-[11px] font-bold text-white">
