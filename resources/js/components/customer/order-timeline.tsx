...
const CANCELLED_BY_CUSTOMER_STEP: TimelineStep = {
    key: 'cancelled_by_customer',
    label: 'Dibatalkan Customer',
    icon: XCircle,
};
const CANCELLED_BY_OUTLET_STEP: TimelineStep = {
    key: 'cancelled_by_outlet',
    label: 'Dibatalkan Outlet',
    icon: XCircle,
};
const REJECTED_STEP: TimelineStep = {
    key: 'rejected_by_outlet',
...
const EXPIRED_STEP: TimelineStep = {
    key: 'expired',
    label: 'Konfirmasi Kadaluarsa',
    icon: Timer,
};
...
export default function OrderTimeline({
    ...
    if (compact) {
        return (
            <CompactTimeline steps={steps} effectiveIndex={effectiveIndex} />
        );
    }

    const currentStepLabel = getStepLabel(
                ...
                <summary className="flex cursor-pointer items-center justify-between p-4 active:opacity-80">
                    <div>
                        ...
                        <div className="mt-0.5 text-xs text-text-muted">
                            {completedCount} dari {totalSteps} langkah
                        </div>
                    </div>
