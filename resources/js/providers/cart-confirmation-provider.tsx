...
import {
    CartConfirmationContext,
    useCartConfirmationState,
} from '@/contexts/cart-confirmation-context';
...
export default function CartConfirmationProvider({ children }: Props) {
    const { isOpen, data, showConfirmation, hideConfirmation } =
        useCartConfirmationState();

    return (
        <CartConfirmationContext.Provider
            value={{ isOpen, data, showConfirmation, hideConfirmation }}
        >
            {children}
