...
function resolveDesignMdPath(cwd = process.cwd()) {
  ...
  if (root) {
return { path: root, contextDir: cwd };
}
  ...
  for (const rel of FALLBACK_DIRS) {
    ...
    if (found) {
return { path: found, contextDir: dir };
}
  }
...
continue;
    ...
    if (!FONT_SIZE_LITERAL_RE.test(raw)) {
continue;
}
...
return true;
  ...
  while (current) {
    if (current.getAttribute?.('hidden') !== null || current.getAttribute?.('aria-hidden') === 'true') {
return true;
}
