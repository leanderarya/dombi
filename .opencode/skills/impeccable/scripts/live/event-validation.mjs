...
function validateManualEditEvent(msg, label) {
  if (!isValidId(msg.id)) {
return label + ': missing or malformed id';
}
...
return label + ': ops must be non-empty array';
  ...
  if (msg.ops.length > 100) {
return label + ': too many ops (max 100)';
}

  for (const op of msg.ops) {
    if (typeof op.ref !== 'string') {
return label + ': op.ref required';
}

    if (typeof op.tag !== 'string') {
return label + ': op.tag required';
}
...
return 'generate: count must be 1-8';
      ...
      if (msg.mode === 'insert') {
return validateInsertGenerate(msg);
}
...
return 'prefetch: missing pageUrl';
    ...
    case 'steer':
      if (!isValidId(msg.id)) {
return 'steer: missing or malformed id';
}
