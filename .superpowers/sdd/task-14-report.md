...
Current verification remains MySQL-blocked; no current scoped-test, migration, or dry-run PASS is claimed. Cutover now outputs `READY` only when parity passes and runtime evidence strictly parses to explicit false; otherwise it outputs `BLOCKED`. Observability counters require atomic cache increment and event writes are deferred after commit; financial truth remains durable outbox. Observability now fails safely when backend unavailable; durable outbox remains financial truth. Post-cutover canonical obligations/attempts are exempt from legacy parity but FK/order/attempt integrity is still checked. Production cutover remains NO-GO until runtime evidence and full-suite blockers clear.
