...
Every payment lifecycle event is emitted through the registered `PaymentObservabilityService` counter/gauge/event registry and mirrored to the `operational` channel. Events use fixed nullable fields: `order_id`, `attempt_id`, `invoice_number`, `request_id`, `gateway_reference`, `mapped_status`, `processing_result`, and `error_reason`. Labels are allowlisted; raw payloads, signatures, credentials, secret material, and arbitrary labels are rejected. Registered events cover creation failures and timeouts, invalid responses/signatures, unknown statuses, amount mismatches, pending-age breaches, reconciliation failures, late payments, duplicate successes, refund ageing, and needs-review.

Before read-only cutover, run `php artisan payments:verify-cutover`. The command fails on missing or duplicate canonical attempts, invoice/order/amount/currency/status mismatches, and any legacy write marker. Its parity report covers every legacy `payment_transactions` row against one canonical `payment_attempts` row by legacy ID, invoice, order, amount, currency, and settlement state. Compare every legacy refund field to one `refund_obligations` row by order, attempt, amount, currency, destination, and lifecycle state. Unclassified, missing, duplicate, amount, or status mismatches block cutover.
