...
| Return create (outlet): submit "tak terjadi apa-apa" lalu error DB sesaat di staging. Backend `/outlet/returns` store OK di local (feature test pass), frontend fix product_id uda deployed, DB staging OK utk halaman lain. Kemungkinan lock contention / queue worker / data staging. Butuh investigasi akses server | Open | Cek queue worker + lock + log staging |
